...
const Deposit: React.FC<DepositProps> = ({ token, onClose }) => {
  ...
  return (
    <div
      className={`w-[350px] rounded-lg flex flex-col items-center py-4 px-5 absolute z-20 top-4 right-4 shadow-lg ${
        isDarkMode
          ? "bg-[#1F1333] text-white border border-[#8280FF]"
          : "bg-[#F8F4FF] text-black border border-[#ECE1FF]"
      }`}
    >
        ...
        <div>
          <div className="flex justify-between mb-1">
            ...
            <span className="text-sm text-gray-400">
              {t("deposit.balance")}:{" "}
              {isLoadingBalance ? t("common.loading") : formattedBalance}
            </span>
          </div>
          <input
            id="amount"
      ...
      >
        {!account ? t("deposit.connectWallet") : t("deposit.deposit")}
      </button>
    </div>
  );
};
